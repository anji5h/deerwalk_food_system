...
export const AddItemService = async (body: any) => {
  ...
  let row = await SelectQuery.from(Item, "item")
...
export const AddItemCategoryService = async (body: any) => {
  ...
  let row = await SelectQuery.select()
  ...
  await InsertQuery.into(ItemCategory).values({ name: data.name }).execute();
};
...
export const AddItemTypeService = async (body: any) => {
  ...
  await InsertQuery.into(ItemType).values({ name: data.name }).execute();
};
